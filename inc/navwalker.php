<?php
/*
 * Class Name: Bigwigs_WP_Bootstrap_Navwalker
...
if ( ! class_exists( 'Bigwigs_WP_Bootstrap_Navwalker' ) ) {
	/**
	 * Bigwigs_WP_Bootstrap_Navwalker class.
	 *
	 * @extends Walker_Nav_Menu
	 */
	class Bigwigs_WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( isset( $args->has_children ) && $args->has_children && 0 === $depth && $args->depth > 1 ) {
				...
				$atts['class'] = 'nav-link unit-item';
			} else {
				$atts['href'] = ! empty( $item->url ) ? $item->url : '#';
				// Items in dropdowns use .dropdown-item instead of .nav-link.
				if ( isset( $args->has_children ) && $args->has_children && $depth > 0 && $depth < 2 ) {
					$atts['class'] = 'dropdown-item unit-item';
				} elseif ( $depth > 0 ) {
			...
			/**
			 * This is the end of the internal nav item.
			 * We need to close the element depending on the type of link or link mod.
			 * 
			 * With no link mod type set this
			 * returns the html markup for displaying the toggle button of dropdown submenu
			 * if this is the parent item of the drop-down submenu
			 * otherwise must be a standard <a> tag
			 */
			if ( '' !== $linkmod_type ) {
				// is linkmod, output the required element opener.
				...
				$item_output .= '</a>';
				if ( isset( $args->has_children ) && $args->has_children && $depth < 2 ) {
					$item_output .= '</a><button type="button" class="btn btn-link dropdown-toggle dropdown-toggle-split" id="'.$item->ID.'" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" data-reference="parent"><span class="sr-only">'.esc_html( 'Toggle Dropdown', 'bigwigs' ).'</span></button>';
				}
			}
		...
		public static function fallback( $args ) {
			if ( current_user_can( 'edit_theme_options' ) ) {
				...
				$fallback_output .= '>';
				$fallback_output .= '<li><a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" title="' . esc_attr__( 'Add a menu', 'bigwigs' ) . '">' . esc_html__( 'Add a menu', 'bigwigs' ) . '</a></li>';
				$fallback_output .= '</ul>';
				if ( $container ) {
					$fallback_output .= '</' . esc_attr( $container ) . '>';
				}
